import numpy as np
from . import utils
from .i_index import i_index_adjusted

def xie_beni_original(X, labels):
	'''
...
def xie_beni(X, labels, adjusted=False, k=0.29289310158571386):
	if adjusted:
		return i_index_adjusted(X, labels, k)
	else:
		return xie_beni_original(X, labels)
